...
const SlotBooking = () => {
  ...
  const handleNext = async (e) => {
    ...
    // If moving from Step 2 to Step 3 (Confirmation), call the API!
    if (step === 2) {
      try {
        ...
        if (response.ok) {
          // Save the actual booking data generated by the backend!
          setBookingResponse(data.booking);
          // Store farmer ID in local storage so Tracking dashboard knows who is logged in
          localStorage.setItem('activeFarmerId', data.booking.farmerId);
          setStep(step + 1);
        } else {
                ...
                <div className="bg-gray-50 border border-gray-200 rounded p-6 mb-8 relative overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-y-6 gap-x-4 relative z-10">
                    <div>
                      ...
                      <p className="text-lg font-extrabold text-govGreen-900">{bookingResponse?.bookingId || 'BK-ERROR'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">Farmer ID</p>
                      <p className="text-lg font-bold text-gray-900">{bookingResponse?.farmerId}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">Crop</p>
                      <p className="text-lg font-bold text-gray-900">{bookingResponse?.crop}</p>
                    </div>
                    <div className="col-span-2 md:col-span-1">
                      <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">Procurement Centre</p>
                      <p className="text-lg font-bold text-gray-900 leading-tight">{bookingResponse?.centre}</p>
                      <p className="text-sm text-gray-600">{bookingResponse?.district}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">Date</p>
                      <p className="text-lg font-bold text-gray-900">{bookingResponse?.date}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">Your Token #</p>
                      <p className="text-2xl font-extrabold text-govGreen-700 bg-green-50 px-2 py-0.5 rounded border border-green-200 inline-block">{bookingResponse?.farmerToken}</p>
                    </div>
                  </div>
                </div>
