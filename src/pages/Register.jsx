...
const Register = () => {
  ...
  const validateStep = (currentStep) => {
    ...
    if (currentStep === 1) {
      ...
      // Land record step no longer requires crop selection
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateStep(4)) {
      ...
      try {
        ...
        if (response.ok) {
          setLoading(false);
          setRegId(data.user.farmerId); // Use the real ID generated by the backend!
          setSuccess(true);
          localStorage.removeItem('farmerRegistrationDraft');
          localStorage.setItem('activeFarmerId', data.user.farmerId); // Auto-login
          window.scrollTo(0, 0);
        } else {
          setLoading(false);
